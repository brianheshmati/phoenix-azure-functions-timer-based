...
SRC_CP_COL = 1522354907467652 # Cath. Prot. Column on 02 sheet
...
DRY_RUN = os.getenv("DRY_RUN_CP", "false").lower() == "true"
...
def index_dest_by_tank_and_row() -> Dict[str, Dict[str, Any]]:
    ...
    while True:
        ...
        for row in batch:
            ...
            if row_val == ROW_VALUE_CP and tank_val not in (None, ""):
...
def build_operations(
    ...
    for srow in source_rows:
        ...
        src_CP_val = str((scells.get(SRC_CP_COL) or {}).get("value") or "").strip()
        ...
        logging.info(f"[Plan] Source row Tank={src_tank_val}, CP={src_CP_val}, NTP Date={src_ntp_date_val}, Contract Days={src_contract_days_val}")
        ...
        for row in candidates:
            ...
            if val == ROW_VALUE_CP:   # all indexed rows should already match
                dest_row = row
                break
        ...
        dest_CP_val = dest_cells.get(DEST_CP_COL, {}).get('value')
        
        mapped_cells: List[Dict[str, Any]] = []
        
        if dest_row is None:
            # INSERT only if source "CP" is "Rquired"
            if src_CP_val == "Required":
                 # Build mapped cell payload        
                for src_col, dest_col in COLUMN_MAP.items():
                    if src_col in scells:
                        mapped_cells.append({"columnId": dest_col, "value": scells[src_col].get("value")})
                
                mapped_cells.append({"columnId": DEST_PRIMERY_COL, "value": ROW_VALUE_CP}) # Primary column
                mapped_cells.append({"columnId": DEST_ORDER_COL, "value": DEST_ORDER_VAL}) # Order
                # Force Row column in destination to CP"
                mapped_cells.append({"columnId": DEST_ROW_COL, "value": ROW_VALUE_CP})
                mapped_cells.append({"columnId": DEST_CP_COL, "value": src_CP_val}) # CP column on 09 sheet with the value from 02 sheet

                inserts.append({"toBottom": True, "cells": mapped_cells})
                logging.info(f"[Plan] INSERT tank={tank_key} (CP = {src_CP_val})")
            else:
                logging.info(f"[Plan] SKIP insert tank={tank_key} (CP= {src_CP_val})")
        else:
            ...
            if(src_CP_val != dest_CP_val):
                mapped_cells.append({"columnId": DEST_CP_COL, "value": src_CP_val}) # update the CP column on 09 sheet with the value from 02 sheet
                logging.info(f"[Plan] UPDATE tank={tank_key} (Turning CP from {dest_CP_val} to {src_CP_val})")
...
def main(mytimer: func.TimerRequest) -> None:
    ...
    try:
        ...
        logging.info(f"[SmartsheetSync] Indexed destination rows (Row='CP'): {len(dest_index)}")
        ...
        logging.exception(f"[identity-CP-sync] FAILED: {ex}")
        raise
