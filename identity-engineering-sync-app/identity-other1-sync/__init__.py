...
SRC_O1_COL = 2085304860888964 # OTHER 1 Column on 02 sheet
...
DEST_O1_COL = 3951701782122372 # OTHER 1 column on 09-Subcontracts sheet
DEST_PRIMERY_COL = 8710388107136900 # Primary column on 09 sheet
DEST_ORDER_COL = 6766451549228932 # Order column on 09 sheet
DEST_ORDER_VAL = "0013 - Other 1" # Order value for Other 1 rows on 09 sheet
DEST_PROJECT_MANAGER_COL = 547613782527876

ROW_VALUE_PROJECT     = "Project"
ROW_VALUE_O1 = "Other 1"  # Row value for Other 1 rows on 09 sheet
...
DRY_RUN = os.getenv("DRY_RUN_O1", "false").lower() == "true"
...
def index_dest_by_tank_and_row() -> Dict[str, Dict[str, Any]]:
    ...
    while True:
        ...
        for row in batch:
            ...
            if row_val == ROW_VALUE_O1 and tank_val not in (None, ""):
...
def build_operations(
    ...
    for srow in source_rows:
        ...
        src_o1_val = str((scells.get(SRC_O1_COL) or {}).get("value") or "").strip()
        ...
        logging.info(f"[Plan] Source row Tank={src_tank_val}, CP={src_o1_val}, NTP Date={src_ntp_date_val}, Contract Days={src_contract_days_val}")
        ...
        for row in candidates:
            ...
            if val == ROW_VALUE_O1:   # all indexed rows should already match
                dest_row = row
                break
        ...
        dest_o1_val = dest_cells.get(DEST_O1_COL, {}).get('value')
        ...
        if dest_row is None:
            ...
            if src_o1_val.casefold() not in [item.casefold() for item in search_list]:
                 # Build mapped cell payload        
                for src_col, dest_col in COLUMN_MAP.items():
                    if src_col in scells:
                        mapped_cells.append({"columnId": dest_col, "value": scells[src_col].get("value")})
                
                mapped_cells.append({"columnId": DEST_PRIMERY_COL, "value": src_o1_val}) # Primary column
                ...
                logging.info(f"[Plan] SKIP insert tank={tank_key} (Other 1 = {src_o1_val})")
        else:
            ...
            if(src_o1_val != dest_o1_val):
                mapped_cells.append({"columnId": DEST_O1_COL, "value": src_o1_val}) # update the Other 1 column on 09 sheet with the value from 02 sheet
                logging.info(f"[Plan] UPDATE tank={tank_key} (Turning Other 1 from {dest_o1_val} to {src_o1_val})")
...
def main(mytimer: func.TimerRequest) -> None:
    ...
    try:
        ...
        logging.info(f"[SmartsheetSync] Indexed destination rows (Row='Other 1'): {len(dest_index)}")
        ...
        logging.exception(f"[identity-other1-sync] FAILED: {ex}")
        raise
