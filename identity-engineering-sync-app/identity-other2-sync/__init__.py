...
SRC_O2_COL = 4337104674574212 # OTHER 2 Column on 02 sheet 
...
DEST_O2_COL = 8455301409492868 # OTHER 2 column on 09-Subcontracts sheet
DEST_PRIMERY_COL = 8710388107136900 # Primary column on 09 sheet
DEST_ORDER_COL = 6766451549228932 # Order column on 09 sheet
DEST_ORDER_VAL = "0013 - Other 2" # Order value for Other 1 rows on 09 sheet
DEST_PROJECT_MANAGER_COL = 547613782527876

ROW_VALUE_PROJECT     = "Project"
ROW_VALUE_O2 = "Other 2"  # Row value for Other 1 rows on 09 sheet
...
DRY_RUN = os.getenv("DRY_RUN_O2", "false").lower() == "true"
...
def index_dest_by_tank_and_row() -> Dict[str, Dict[str, Any]]:
    ...
    while True:
        ...
        for row in batch:
            ...
            if row_val == ROW_VALUE_O2 and tank_val not in (None, ""):
...
def build_operations(
    ...
    for srow in source_rows:
        ...
        src_o2_val = str((scells.get(SRC_O2_COL) or {}).get("value") or "").strip()
        ...
        logging.info(f"[Plan] Source row Tank={src_tank_val}, CP={src_o2_val}, NTP Date={src_ntp_date_val}, Contract Days={src_contract_days_val}")
        ...
        for row in candidates:
            ...
            if val == ROW_VALUE_O2:   # all indexed rows should already match
                dest_row = row
                break
        ...
        dest_o2_val = dest_cells.get(DEST_O2_COL, {}).get('value')
        
        mapped_cells: List[Dict[str, Any]] = []
        
        if dest_row is None:
            # INSERT only if source "Other 1" is not "NR", "Not Required", "None" or "N/A"
            search_list = ["NR", "Not Required", "None", "N/A", ""]
            
            if src_o2_val.casefold() not in [item.casefold() for item in search_list]:
                 # Build mapped cell payload        
                for src_col, dest_col in COLUMN_MAP.items():
                    if src_col in scells:
                        mapped_cells.append({"columnId": dest_col, "value": scells[src_col].get("value")})
                
                mapped_cells.append({"columnId": DEST_PRIMERY_COL, "value": src_o2_val}) # Primary column
                mapped_cells.append({"columnId": DEST_ORDER_COL, "value": DEST_ORDER_VAL}) # Order
                # Force Row column in destination to Other 1"
                mapped_cells.append({"columnId": DEST_ROW_COL, "value": ROW_VALUE_O2})
                mapped_cells.append({"columnId": DEST_O2_COL, "value": src_o2_val}) # Other 2 column on 09 sheet with the value from 02 sheet

                inserts.append({"toBottom": True, "cells": mapped_cells})
                logging.info(f"[Plan] INSERT tank={tank_key} (Other 2 = {src_o2_val})")
            else:
                logging.info(f"[Plan] SKIP insert tank={tank_key} (Other 2 = {src_o2_val})")
        else:
            ...
            if(src_o2_val != dest_o2_val):
                mapped_cells.append({"columnId": DEST_O2_COL, "value": src_o2_val}) # update the Other 2 column on 09 sheet with the value from 02 sheet
                logging.info(f"[Plan] UPDATE tank={tank_key} (Turning Other 1 from {dest_o2_val} to {src_o2_val})")
...
def main(mytimer: func.TimerRequest) -> None:
    ...
    try:
        ...
        logging.info(f"[SmartsheetSync] Indexed destination rows (Row='Other 2'): {len(dest_index)}")
        ...
        logging.exception(f"[identity-other2-sync] FAILED: {ex}")
        raise
