...
SRC_FENCE_COL = 396455000625028 # FENCE Column on 02 sheet
...
DRY_RUN = os.getenv("DRY_RUN_FENCE", "false").lower() == "true"
...
def index_dest_by_tank_and_row() -> Dict[str, Dict[str, Any]]:
    ...
    while True:
        ...
        for row in batch:
            ...
            if row_val == ROW_VALUE_FENCE and tank_val not in (None, ""):
...
def build_operations(
    ...
    for srow in source_rows:
        ...
        src_fence_val = str((scells.get(SRC_FENCE_COL) or {}).get("value") or "").strip()
        ...
        logging.info(f"[Plan] Source row Tank={src_tank_val}, Fence={src_fence_val}, NTP Date={src_ntp_date_val}, Contract Days={src_contract_days_val}")
        ...
        for row in candidates:
            ...
            if val == ROW_VALUE_FENCE:   # all indexed rows should already match
                dest_row = row
                break
        ...
        dest_fence_val = dest_cells.get(DEST_FENCE_COL, {}).get('value')
        
        mapped_cells: List[Dict[str, Any]] = []
        
        if dest_row is None:
            # INSERT only if source "Fence" is "Phoenix" or "Subcontractor"
            if src_fence_val == "Phoenix" or src_fence_val == "Subcontractor":
                 # Build mapped cell payload        
                for src_col, dest_col in COLUMN_MAP.items():
                    if src_col in scells:
                        mapped_cells.append({"columnId": dest_col, "value": scells[src_col].get("value")})
                
                mapped_cells.append({"columnId": DEST_PRIMERY_COL, "value": ROW_VALUE_FENCE}) # Primary column
                mapped_cells.append({"columnId": DEST_ORDER_COL, "value": DEST_ORDER_VAL}) # Order
                # Force Row column in destination to "Fence"
                mapped_cells.append({"columnId": DEST_ROW_COL, "value": ROW_VALUE_FENCE})
                mapped_cells.append({"columnId": DEST_FENCE_COL, "value": src_fence_val}) # Fence column on 09 sheet with the value from 02 sheet

                inserts.append({"toBottom": True, "cells": mapped_cells})
                logging.info(f"[Plan] INSERT tank={tank_key} (Fence = {src_fence_val})")
            else:
                logging.info(f"[Plan] SKIP insert tank={tank_key} (Fence = {src_fence_val})")
        else:
            ...
            if(src_fence_val != dest_fence_val):
                mapped_cells.append({"columnId": DEST_FENCE_COL, "value": src_fence_val}) # update the FENCE column on 09 sheet with the value from 02 sheet
                logging.info(f"[Plan] UPDATE tank={tank_key} (Turning Fence from {dest_fence_val} to {src_fence_val})")
...
def main(mytimer: func.TimerRequest) -> None:
    ...
    try:
        ...
        logging.info(f"[SmartsheetSync] Indexed destination rows (Row='Fence'): {len(dest_index)}")
        ...
        logging.exception(f"[identity-Fence-sync] FAILED: {ex}")
        raise
