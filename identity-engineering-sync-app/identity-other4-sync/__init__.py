...
SRC_O4_COL = 2507517325954948 # OTHER 4 Column on 02 sheet
...
DEST_O4_COL = 4796126712254340 # OTHER 4 column on 09-Subcontracts sheet
DEST_PRIMERY_COL = 8710388107136900 # Primary column on 09 sheet
DEST_ORDER_COL = 6766451549228932 # Order column on 09 sheet
DEST_ORDER_VAL = "0013 - Other 4" # Order value for Other 3 rows on 09 sheet
DEST_PROJECT_MANAGER_COL = 547613782527876

ROW_VALUE_PROJECT     = "Project"
ROW_VALUE_O4 = "Other 4"  # Row value for Other 1 rows on 09 sheet
...
DRY_RUN = os.getenv("DRY_RUN_O4", "false").lower() == "true"
...
def index_dest_by_tank_and_row() -> Dict[str, Dict[str, Any]]:
    ...
    while True:
        ...
        for row in batch:
            ...
            if row_val == ROW_VALUE_O4 and tank_val not in (None, ""):
...
def build_operations(
    ...
    for srow in source_rows:
        ...
        src_o4_val = str((scells.get(SRC_O4_COL) or {}).get("value") or "").strip()
        ...
        logging.info(f"[Plan] Source row Tank={src_tank_val}, CP={src_o4_val}, NTP Date={src_ntp_date_val}, Contract Days={src_contract_days_val}")
        ...
        for row in candidates:
            ...
            if val == ROW_VALUE_O4:   # all indexed rows should already match
                dest_row = row
                break
        
        dest_cells = cells_array_to_dict(dest_row.get("cells", [])) if dest_row else {}
        
        dest_o4_val = dest_cells.get(DEST_O4_COL, {}).get('value')
        ...
        if dest_row is None:
            ...
            if src_o4_val.casefold() not in [item.casefold() for item in search_list]:
                ...
                mapped_cells.append({"columnId": DEST_O4_COL, "value": src_o4_val}) # Other 4 column on 09 sheet with the value from 02 sheet

                inserts.append({"toBottom": True, "cells": mapped_cells})
                logging.info(f"[Plan] INSERT tank={tank_key} (Other 2 = {src_o4_val})")
            else:
                logging.info(f"[Plan] SKIP insert tank={tank_key} (Other 2 = {src_o4_val})")
        else:
            ...
            if(src_o4_val != dest_o4_val):
                mapped_cells.append({"columnId": DEST_O4_COL, "value": src_o4_val}) # update the Other 4 column on 09 sheet with the value from 02 sheet
                logging.info(f"[Plan] UPDATE tank={tank_key} (Turning Other 1 from {dest_o4_val} to {src_o4_val})")
...
def main(mytimer: func.TimerRequest) -> None:
    ...
    try:
        ...
        logging.info(f"[SmartsheetSync] Indexed destination rows (Row='Other 4'): {len(dest_index)}")
        ...
        logging.exception(f"[identity-other4-sync] FAILED: {ex}")
        raise
