...
SRC_HVAC_COL = 3774154721152900 # HVAC Column on 02 sheet
...
DRY_RUN = os.getenv("DRY_RUN_HVAC", "false").lower() == "true"
...
def index_dest_by_tank_and_row() -> Dict[str, Dict[str, Any]]:
    ...
    while True:
        ...
        for row in batch:
            ...
            if row_val == ROW_VALUE_HVAC and tank_val not in (None, ""):
...
def build_operations(
    ...
    for srow in source_rows:
        ...
        src_hvac_val = str((scells.get(SRC_HVAC_COL) or {}).get("value") or "").strip()
        ...
        for row in candidates:
            ...
            if val == ROW_VALUE_HVAC:   # all indexed rows should already match
                dest_row = row
                break
        ...
        dest_hvac_val = dest_cells.get(DEST_HVAC_COL, {}).get('value')
        
        mapped_cells: List[Dict[str, Any]] = []
        
        if dest_row is None:
            # INSERT only if source "HVAC" is "Phoenix or Subcontractor"
            if src_hvac_val == "Phoenix" or src_hvac_val == "Subcontractor":
                ...
                logging.info(f"[Plan] INSERT tank={tank_key} (HVAC = {src_hvac_val})")
            else:
                logging.info(f"[Plan] SKIP insert tank={tank_key} (HVAC= {src_hvac_val})")
        else:
            # UPDATE always if there are diffs
            
            dest_hvac_val = dest_cells.get(DEST_HVAC_COL, {}).get('value')
            
            if(src_hvac_val != dest_hvac_val):
                mapped_cells.append({"columnId": DEST_HVAC_COL, "value": src_hvac_val}) # update the HVAC column on 09 sheet with the value from 02 sheet
                logging.info(f"[Plan] UPDATE tank={tank_key} (Turning HVAC from {dest_hvac_val} to {src_hvac_val})")
...
def main(mytimer: func.TimerRequest) -> None:
    ...
    try:
        ...
        logging.info(f"[SmartsheetSync] Indexed destination rows (Row='HVAC'): {len(dest_index)}")
