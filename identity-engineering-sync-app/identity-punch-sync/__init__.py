...
DEST_SHEET_ID   = 2176504579444612  # 025-Punch List - hardcoded

# Source column IDs
SRC_TANK_COL        = 3633417232797572
SRC_ROW_COL         = 537192488980356
SRC_ORDER_COL       = 8699966813589380 # columnId for "Order" here
SRC_PUNCH_COL = 8966084623028100 # PUNCH Column on 02 sheet
...
DEST_TANK_COL = 7943586976583556
DEST_ROW_COL  = 6536212093030276
DEST_NTP_DATE_COL  = 8506536930004868
DEST_CONTRACT_DAYS_COL = 343762605395844
DEST_NTP_COMPLETION_DATE_COL = 4847362232766340
DEST_PUNCH_COL = 203025117040516 # PUNCH column on DEST sheet
DEST_PRIMERY_COL = 7380637023162244 # Primary column on DEST sheet
DEST_ORDER_COL = 2032612465659780 # Order column on DEST sheet
DEST_ORDER_VAL = "0025 - Punch List" # Order value for PUNCH rows on DEST sheet
DEST_PROJECT_MANAGER_COL = 5410312186187652

ROW_VALUE_PROJECT     = "Project"
ROW_VALUE_PUNCH = "Punch List"  # Row value for Punch rows on DEST sheet
ORDER_VALUE_PROJECT   = "0000 - Project"

SRC_DEST_COLUMN_MAP : Dict[int, int] = {
    3633417232797572: 7943586976583556,  # Tank #
    8137016860168068: 625237582106500,  # Site name
    818667465691012:  5128837209476996,  # City
    5322267093061508: 2877037395791748,  # State
    2155673605066628: 1751137488949124,  # Size
    6659273232437124: 6254737116319620,  # Type
    4618579651284868: 5410312186187652,   # Project manager
    5885217046482820: 3158512372502404,  # Estimator
    # 6448166999904132: 4002937302634372,  # Contract - date
    # 3844523465330564: 8506536930004868,  # NTP date
    # 8348123092701060: 343762605395844,  # Contract days
    # 1029773698224004: 4847362232766340,  # NTP completion date
    # 5533373325594500: 2595562419081092,   # LDs
    4407473418751876: 8225061953294212,  # Engineering firm
    8911073046122372: 906712558817156,  # Owner
    1381617419112324: 7662111999872900,  # Bid #
}
...
DRY_RUN = os.getenv("DRY_RUN_PUNCH", "false").lower() == "true"
...
def index_dest_by_tank_and_row() -> Dict[str, Dict[str, Any]]:
    ...
    We only include rows whose 'Row' column is 'Paint' so
    ...
    while True:
        ...
        for row in batch:
            ...
            if row_val == ROW_VALUE_PUNCH and tank_val not in (None, ""):
...
def build_operations(
    ...
    for srow in source_rows:
        ...
        src_punch_val = str((scells.get(SRC_PUNCH_COL) or {}).get("value") or "").strip()
        ...
        logging.info(f"[Plan] Source row Tank={src_tank_val}, CP={src_punch_val}, NTP Date={src_ntp_date_val}, Contract Days={src_contract_days_val}")
        ...
        for row in candidates:
            ...
            if val == ROW_VALUE_PUNCH:   # all indexed rows should already match
                dest_row = row
                break
        ...
        dest_punch_val = dest_cells.get(DEST_PUNCH_COL, {}).get('value')
        
        mapped_cells: List[Dict[str, Any]] = []
        
        if dest_row is None:
            # INSERT only if source "Punch" is "Rquired"
            if src_punch_val == "Required":
                 # Build mapped cell payload        
                for src_col, dest_col in COLUMN_MAP.items():
                    if src_col in scells:
                        mapped_cells.append({"columnId": dest_col, "value": scells[src_col].get("value")})
                
                mapped_cells.append({"columnId": DEST_PRIMERY_COL, "value": ROW_VALUE_PROJECT}) # Primary column
                mapped_cells.append({"columnId": DEST_ORDER_COL, "value": DEST_ORDER_VAL}) # Order
                # Force Row column in destination to Other 1"
                mapped_cells.append({"columnId": DEST_ROW_COL, "value": ROW_VALUE_PUNCH})
                mapped_cells.append({"columnId": DEST_PUNCH_COL, "value": src_punch_val}) # Punch List column on 025 with the value from 02 sheet

                inserts.append({"toBottom": True, "cells": mapped_cells})
                logging.info(f"[Plan] INSERT tank={tank_key} (PUNCH = {src_punch_val})")
            else:
                logging.info(f"[Plan] SKIP insert tank={tank_key} (PUNCH = {src_punch_val})")
        else:
            ...
            if(src_punch_val != dest_punch_val):
                mapped_cells.append({"columnId": DEST_PUNCH_COL, "value": src_punch_val}) # update the PUNCH column on 025 sheet with the value from 02 sheet
                logging.info(f"[Plan] UPDATE tank={tank_key} (Turning PUNCH from {dest_punch_val} to {src_punch_val})")
...
def main(mytimer: func.TimerRequest) -> None:
    ...
    try:
        ...
        logging.exception(f"[identity-punch-sync] FAILED: {ex}")
        raise
