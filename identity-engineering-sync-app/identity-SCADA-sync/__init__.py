...
SRC_SCADA_COL = 4055629697863556 # SCADA Column on 02 sheet
...
DRY_RUN = os.getenv("DRY_RUN_SCADA", "false").lower() == "true"
...
def index_dest_by_tank_and_row() -> Dict[str, Dict[str, Any]]:
    ...
    while True:
        ...
        for row in batch:
            ...
            if row_val == ROW_VALUE_SCADA and tank_val not in (None, ""):
...
def build_operations(
    ...
    for srow in source_rows:
        ...
        src_scada_val = str((scells.get(SRC_SCADA_COL) or {}).get("value") or "").strip()
        ...
        logging.info(f"[Plan] Source row Tank={src_tank_val}, SCADA={src_scada_val}, NTP Date={src_ntp_date_val}, Contract Days={src_contract_days_val}")
        ...
        for row in candidates:
            ...
            if val == ROW_VALUE_SCADA:   # all indexed rows should already match
                dest_row = row
                break
        ...
        dest_scada_val = dest_cells.get(DEST_SCADA_COL, {}).get('value')
        
        mapped_cells: List[Dict[str, Any]] = []
        
        if dest_row is None:
            # INSERT only if source "SCADA" is "Included by Electrical Subcontractor, or Subcontractor"
            if src_scada_val == "Included by Electrical Subcontractor" or src_scada_val == "Subcontractor":
                 # Build mapped cell payload        
                for src_col, dest_col in COLUMN_MAP.items():
                    if src_col in scells:
                        mapped_cells.append({"columnId": dest_col, "value": scells[src_col].get("value")})
                
                mapped_cells.append({"columnId": DEST_PRIMERY_COL, "value": ROW_VALUE_SCADA}) # Primary column
                mapped_cells.append({"columnId": DEST_ORDER_COL, "value": DEST_ORDER_VAL}) # Order
                # Force Row column in destination to SCADA"
                mapped_cells.append({"columnId": DEST_ROW_COL, "value": ROW_VALUE_SCADA})
                mapped_cells.append({"columnId": DEST_SCADA_COL, "value":src_scada_val}) # SCADA column on 09 sheet with the value from 02 sheet

                inserts.append({"toBottom": True, "cells": mapped_cells})
                logging.info(f"[Plan] INSERT tank={tank_key} (SCADA = {src_scada_val})")
            else:
                logging.info(f"[Plan] SKIP insert tank={tank_key} (SCADA= {src_scada_val})")
        else:
            ...
            if(src_scada_val != dest_scada_val):
                mapped_cells.append({"columnId": DEST_SCADA_COL, "value": src_scada_val}) # update the SCADA column on 09 sheet with the value from 02 sheet
                logging.info(f"[Plan] UPDATE tank={tank_key} (Turning SCADA from {dest_scada_val} to {src_scada_val})")
...
def main(mytimer: func.TimerRequest) -> None:
    ...
    try:
        ...
        logging.info(f"[SmartsheetSync] Indexed destination rows (Row='SCADA'): {len(dest_index)}")
        ...
        logging.exception(f"[identity-SCADA-sync] FAILED: {ex}")
        raise
