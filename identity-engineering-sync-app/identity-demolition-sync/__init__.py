...
SRC_DEMO_COL = 2648254814310276 # DEMOLITION Column on 02 sheet
...
DRY_RUN = os.getenv("DRY_RUN_DEMOLITION", "false").lower() == "true"
...
def index_dest_by_tank_and_row() -> Dict[str, Dict[str, Any]]:
    ...
    while True:
        ...
        for row in batch:
            ...
            if row_val == ROW_VALUE_DEMO and tank_val not in (None, ""):
...
def build_operations(
    ...
    for srow in source_rows:
        ...
        src_demo_val = str((scells.get(SRC_DEMO_COL) or {}).get("value") or "").strip()
        ...
        logging.info(f"[Plan] Source row Tank={src_tank_val}, CP={src_demo_val}, NTP Date={src_ntp_date_val}, Contract Days={src_contract_days_val}")
        ...
        for row in candidates:
            ...
            if val == ROW_VALUE_DEMO:   # all indexed rows should already match
                dest_row = row
                break
        ...
        dest_demo_val = dest_cells.get(DEST_DEMO_COL, {}).get('value')
        
        mapped_cells: List[Dict[str, Any]] = []
        
        if dest_row is None:
            # INSERT only if source "Demolition" is "Rquired"
            if src_demo_val == "Required":
                 # Build mapped cell payload        
                for src_col, dest_col in COLUMN_MAP.items():
                    if src_col in scells:
                        mapped_cells.append({"columnId": dest_col, "value": scells[src_col].get("value")})
                
                mapped_cells.append({"columnId": DEST_PRIMERY_COL, "value": ROW_VALUE_DEMO}) # Primary column
                mapped_cells.append({"columnId": DEST_ORDER_COL, "value": DEST_ORDER_VAL}) # Order
                # Force Row column in destination to DEMO"
                mapped_cells.append({"columnId": DEST_ROW_COL, "value": ROW_VALUE_DEMO})
                mapped_cells.append({"columnId": DEST_DEMO_COL, "value": src_demo_val}) # Demolition column on 09 sheet with the value from 02 sheet

                inserts.append({"toBottom": True, "cells": mapped_cells})
                logging.info(f"[Plan] INSERT tank={tank_key} (Demolition = {src_demo_val})")
            else:
                logging.info(f"[Plan] SKIP insert tank={tank_key} (Demolition= {src_demo_val})")
        else:
            ...
            if(src_demo_val != dest_demo_val):
                mapped_cells.append({"columnId": DEST_DEMO_COL, "value": src_demo_val}) # update the Demolition column on 09 sheet with the value from 02 sheet
                logging.info(f"[Plan] UPDATE tank={tank_key} (Turning Demolition from {dest_demo_val} to {src_demo_val})")
...
def main(mytimer: func.TimerRequest) -> None:
    ...
    try:
        ...
        logging.info(f"[SmartsheetSync] Indexed destination rows (Row='Demolition'): {len(dest_index)}")
        ...
        logging.exception(f"[identity-demolition-sync] FAILED: {ex}")
        raise
